...
import React, { useState, useMemo } from 'react';
import { ChevronLeft, ChevronRight, Calendar, Filter, Plus, Edit3, Trash2 } from 'lucide-react';

const MilestoneReportingComponent = () => {
  ...
  const [projects] = useState([
    {
      ...
      color: '#3B82F6',
      milestones: [
        { id: 1, name: 'Requirements Analysis', date: '2025-01-15', status: 'completed' },
        { id: 2, name: 'UI/UX Design', date: '2025-03-20', status: 'in-progress' },
        { id: 3, name: 'Backend Development', date: '2025-05-15', status: 'upcoming' },
        { id: 4, name: 'Testing Phase', date: '2025-07-30', status: 'upcoming' },
        { id: 5, name: 'Launch', date: '2025-09-15', status: 'upcoming' }
      ]
    },
    {
      id: 2,
      ...
      color: '#10B981',
      milestones: [
        { id: 6, name: 'Market Research', date: '2024-12-10', status: 'completed' },
        { id: 7, name: 'Prototype', date: '2025-02-28', status: 'in-progress' },
        { id: 8, name: 'Beta Testing', date: '2025-06-10', status: 'upcoming' },
        { id: 9, name: 'App Store Release', date: '2025-08-20', status: 'upcoming' }
      ]
    },
    {
      id: 3,
      ...
      color: '#8B5CF6',
      milestones: [
        { id: 10, name: 'Architecture Design', date: '2025-04-01', status: 'upcoming' },
        { id: 11, name: 'Data Pipeline', date: '2025-06-15', status: 'upcoming' },
        { id: 12, name: 'Dashboard Development', date: '2025-08-30', status: 'upcoming' },
        { id: 13, name: 'Production Deploy', date: '2025-11-15', status: 'upcoming' }
      ]
    },
    {
      id: 4,
      ...
      color: '#F59E0B',
      milestones: [
        { id: 14, name: 'Assessment', date: '2024-11-30', status: 'completed' },
        { id: 15, name: 'Infrastructure Setup', date: '2025-01-30', status: 'completed' },
        { id: 16, name: 'Data Migration', date: '2025-04-15', status: 'upcoming' },
        { id: 17, name: 'Go Live', date: '2025-06-30', status: 'upcoming' }
      ]
    }
  ]);

  // Generate months for the timeline based on financial year
  const generateTimeline = useMemo(() => {
    const months = [];
    const years = new Map();
    ...
    const financialYearStart = new Date(parseInt(startYear), 3, 1); // April 1st
    
    for (let i = 0; i < 12; i++) {
      const date = new Date(financialYearStart);
      date.setMonth(financialYearStart.getMonth() + i);
  ...
  // Filter projects based on financial year
  const filteredProjects = useMemo(() => {
    const [startYear] = selectedFinancialYear.split('-');
    const fyStart = new Date(parseInt(startYear), 3, 1); // April 1st
    const fyEnd = new Date(parseInt(startYear) + 1, 2, 31); // March 31st next year
    
    return projects.filter(project => {
      // Check if project has any milestones within the selected financial year
      return project.milestones.some(milestone => {
        const milestoneDate = new Date(milestone.date);
        return milestoneDate >= fyStart && milestoneDate <= fyEnd;
      });
    });
    ...
    <div className="w-full max-w-full bg-white rounded-lg shadow-sm border border-gray-200">
      ...
      <div className="p-2 border-b border-gray-200">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            {/* <h2 className="text-2xl font-semibold text-gray-900">Project Milestones</h2> */}
            {/* <p className="text-gray-600 mt-1">Track project progress and upcoming deadlines</p> */}
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            <select 
              ...
              className="px-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 min-w-40"
            >
      ...
      <div className="overflow-x-auto">
        <div className="min-w-max">
          ...
          <div className="flex border-b border-gray-200 bg-gray-50">
            ...
            {Array.from(generateTimeline.years.entries()).map(([year, monthKeys]) => (
              <div 
                key={year}
                className="border-r border-gray-200 bg-gray-100 px-2 py-3 text-center font-semibold text-gray-700"
                style={{ width: `${monthKeys.length * 120}px` }}
          ...
          {/* Project Rows */}
          {filteredProjects.map((project) => (
            <div key={project.id} className="flex border-b border-gray-100 hover:bg-gray-50 transition-colors">
              {/* Project Info */}
              <div className="w-64 px-4 py-4 border-r border-gray-200 flex-shrink-0">
                <div className="flex items-center gap-3">
                  <div 
              ...
              {generateTimeline.months.map((month) => (
                ...
                >
                  {/* Current month indicator */}
                  {month.key === `${new Date().getFullYear()}-${String(new Date().getMonth() + 1).padStart(2, '0')}` && (
                    <div className="absolute inset-0 bg-blue-50 opacity-50"></div>
                  )}
